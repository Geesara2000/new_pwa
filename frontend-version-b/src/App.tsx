...
import { ShoppingBag, ShoppingCart, CreditCard, Home, RefreshCw, Wifi, WifiOff } from 'lucide-react';
import { Product, CartItem, ApiMetric, CacheLog } from './types';
import { fetchWithMetrics } from './apiHelper';
import { queueOfflineOrder, getQueuedOrders, removeQueuedOrder, QueuedOrder } from './indexedDbHelper';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/sw-fixed.js')
        .then(reg => {
          console.log('Service Worker Registered (Version B)', reg);
        })
        .catch(err => console.error('Service Worker Registration Failed', err));
    }

    // Set up BroadcastChannel listener for PWA Cache logs
    const swChannel = new BroadcastChannel('sw-cache-channel');
    const handleSWMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'CACHE_LOG') {
        const log = event.data.payload as CacheLog;
        setCacheLogs(prev => [log, ...prev].slice(0, 10));
        
        // Expose to window for Playwright metrics collection
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    if (!isOnline) {
      // Queue order offline inside IndexedDB
      await queueOfflineOrder(payload);
      setSuccessMessage('Offline order stored. It will sync automatically when online.');
      setCart([]);
      setName('');
      setEmail('');
      setActiveTab('home');
    ...
    try {
      ...
      // Fallback to offline store if request throws network error
      await queueOfflineOrder(payload);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 shadow-lg">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-2 cursor-pointer" onClick={() => setActiveTab('home')}>
            <div className="bg-emerald-600 p-2 rounded-lg text-white shadow-lg shadow-emerald-500/30">
            ...
            <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">
              Fixed Strategy PWA <span className="text-sm font-semibold text-slate-500">(Version B)</span>
            </span>
          </div>
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-1.5 text-sm">
              ...
              {offlineQueueLength > 0 && (
                <span className="ml-2 bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full text-xs font-semibold">
            ...
            <nav className="flex space-x-1">
              <button
                ...
                className={`flex items-center space-x-1 px-4 py-2 rounded-lg transition ${activeTab === 'home' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-4 py-2 rounded-lg transition ${activeTab === 'products' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-4 py-2 rounded-lg transition relative ${activeTab === 'cart' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-4 py-2 rounded-lg transition ${activeTab === 'checkout' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
              >
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-8">
        {successMessage && (
          <div className="mb-6 p-4 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 rounded-xl flex items-center justify-between">
            <span>{successMessage}</span>
            <button onClick={() => setSuccessMessage(null)} className="font-bold hover:text-emerald-200">×</button>
          </div>
        )}
        ...
        {activeTab === 'home' && (
          <div className="py-12 text-center">
            <h1 className="text-5xl font-extrabold tracking-tight mb-4 bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">
              Fixed Caching Strategy PWA
            </h1>
            <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8">
              This variant (Version B) registers a custom Service Worker utilizing the **Stale-While-Revalidate** algorithm for all network assets and API payloads, enabling offline browsing and seamless queueing.
            </p>
            <button
              onClick={() => setActiveTab('products')}
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-semibold px-8 py-3 rounded-xl shadow-lg shadow-emerald-600/20 hover:shadow-emerald-600/30 transition-all hover:-translate-y-0.5"
            >
                  ...
                  <div key={product.id} className="bg-slate-950/40 border border-slate-800/80 rounded-2xl overflow-hidden hover:border-slate-700/80 transition flex flex-col">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="flex items-center justify-between mt-auto">
                        <span className="text-xl font-bold text-emerald-400">${parseFloat(product.price).toFixed(2)}</span>
                        <button
            ...
            {cart.length === 0 ? (
              ...
              <div className="bg-slate-950/40 border border-slate-800/80 rounded-2xl p-6">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-800">
                  ...
                  <span className="text-2xl font-bold text-emerald-400">${cartTotal.toFixed(2)}</span>
                </div>
              ...
              <form onSubmit={handleCheckout} className="bg-slate-950/40 border border-slate-800/80 rounded-2xl p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-slate-200 focus:border-emerald-500 focus:outline-none"
                ...
                <div className="pt-4">
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-3 rounded-xl transition disabled:opacity-50"
                  >
      ...
      <footer className="bg-slate-950 border-t border-slate-800 p-4 text-xs font-mono">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {localMetrics.length === 0 ? (
                ...
                localMetrics.map((m, idx) => (
                  <div key={idx} className="text-slate-400">
                    [{m.statusCode}] {m.method} {m.url.replace('http://127.0.0.1:8000', '')} -{' '}
          ...
          <div>
            <span className="text-teal-400 font-bold">PWA Cache Engine Hits/Misses</span>
            <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
              {cacheLogs.length === 0 ? (
                <span className="text-slate-500">No cache events logged.</span>
              ) : (
                cacheLogs.map((log, idx) => (
                  <div key={idx} className="text-slate-400">
                    [{log.event.toUpperCase()}] {log.resourceName} -{' '}
                    <span className="text-slate-500">{log.strategyUsed}</span>{' '}
                    {log.responseTime && <span className="text-emerald-400">{log.responseTime.toFixed(2)}ms</span>}
                  </div>
                ))
              )}
            </div>
